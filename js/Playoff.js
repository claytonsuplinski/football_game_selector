	...
	sorted_teams.forEach(function(team){
		if(team.conference == "AFC"){
			...
			if(division_index != -1){
				...
				if(afc_division < 2){
					self.afc.divisional[2 + afc_division] = tmp_team;
				}
				else{
					self.afc.wild_card[5-afc_division] = tmp_team;
				}
				afc_division++;
				...
				afc_wild_card++;
				self.afc.wild_card[2 - afc_wild_card] = tmp_team;
			}
		}
			...
			if(division_index != -1){
				...
				if(nfc_division < 2){
					self.nfc.divisional[2 + nfc_division] = tmp_team;
				}
				else{
					self.nfc.wild_card[5-nfc_division] = tmp_team;
				}
				nfc_division++;
				tmp_divisions.splice(division_index, 1);
			}
				...
				nfc_wild_card++;
				self.nfc.wild_card[2 - nfc_wild_card] = tmp_team;
			}
		}
	});
...
Playoff.prototype.display = function(){
	...
	//alert(JSON.stringify(this.afc.wild_card));
