...
function load_team_schedule(code){
	...
	schedule_data.forEach(function (week, week_number){
		...
		if(game.length > 0){
			...
			tmp_html += '<div class="col-xs-5 col-lg-5 team-schedule-week" onclick="select_game('+game_index+', 0, '+week_number+');load_team_schedule(\''+code+'\');" style="background-color:'+away_team.color+';background-image:url(\'./teamIcons/'+away_team.code+'.png\');"><div class="hidden-xs">'+away_team.code+'</div></div>';
			...
			tmp_html += '<div class="col-xs-5 col-lg-5 team-schedule-week" onclick="select_game('+game_index+', 1, '+week_number+');load_team_schedule(\''+code+'\');" style="background-color:'+home_team.color+';background-image:url(\'./teamIcons/'+home_team.code+'.png\');"><div class="hidden-xs">'+home_team.code+'</div></div>';
		}
		else{//Bye Week
			tmp_html += '<div class="col-xs-12 team-schedule-vs">BYE</div>';
		}
	});
	$("#standings-title").html(team.code + "&nbsp;&nbsp;<button class='btn btn-primary btn-xs' onclick='load_schedule();'>Back</button>");
	$("#standings-container").html(tmp_html);
}
